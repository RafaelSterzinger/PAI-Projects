...
class MLPActorCritic(nn.Module):
    ...
    def step(self, state):
        ...
        with torch.no_grad():
            pi, logp_action = self.pi.forward(state)
            action = pi.sample()
...
class Agent:
    ...
    def train(self):
        ...
        for epoch in range(epochs):
            ...
            if epoch == 0:
                ret_sum = data['ret']
                tdres_sum = data['tdres']
            else:
                ret_sum += data['ret']
                tdres_sum += data['tdres']
            
            policy_history_tensor = Variable(torch.tensor(policy_history), requires_grad=True)
            v_history_tensor = Variable(torch.tensor(v_history), requires_grad = True)
            
            pi_optimizer.zero_grad()
            loss_pi = torch.sum(torch.mul(policy_history_tensor, data['tdres']))*(-1)

            print("Policy Loss", loss_pi)
            ...
            for _ in range(100):
                ...
                loss_v = mse_loss(v_history_tensor, (ret_sum)/(epoch+1))
                loss_v.backward()
                v_optimizer.step()
